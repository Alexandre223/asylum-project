...
**gen log per capita*
gen log_dyadic_decisions_pc = log(((yearly_dyadic_decisions_mean + 1) / pop_destination))
...
* generate logs 
gen log_dest_decisions_pc = log((yearly_dest_decisions_mean + 1)/ pop_destination)


* 2, generate log variables
...
*	
sort destination origin year quarter
by destination origin: gen lag_log_firsttimeapp_pc = log_firsttimeapp_pc[_n-1]
...
* 3, generate rescaled variables and post 2007 dummy
...
label variable lag_log_firsttimeapp_pc "Log first-time applications per capita in previous quarter"
...
* 5, drop origin countries that are not in the top 90% of any sample used at the moment
